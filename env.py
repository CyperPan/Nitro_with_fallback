...
class Environment():
    ...
    def reset_aws_request_id_list(self):
        self.aws_request_id_list = []
    ...
    def init_trainer_config(self):
        ...
        try:
            ...
            pass  # Ray 2.8 stock doesn't support this param
        ...
        if self.algo_name in ["ppo", "ppo_serverless"]:
            ...
            trainer_config = trainer_config.training(
                ...
                # learner_queue_size=config.num_rollout_workers_serverful,
                # replay_buffer_num_slots=config.num_rollout_workers_serverful,
            )
    ...
    def reset_trainer(self):
        ...
        print(f"[DEBUG] num_rollout_workers={self.trainer_config.num_rollout_workers}", flush=True)
        print(f"[DEBUG] evaluation_num_workers={self.trainer_config.evaluation_num_workers}", flush=True)
        print(f"[DEBUG] env={self.trainer_config.env}", flush=True)
    ...
    def get_mask(
        ...
        return torch.zeros(1, config.action_dim)
    ...
    def get_reward(
        ...
        reward = - config.beta * duration
        if done:
            eval_reward_mean = info["eval_reward_mean"]
            if eval_reward_mean < self.target_reward:
                reward = reward + (self.max_eval_reward - self.target_reward)
    ...
    def step(
        ...
        # Update action
        num_rollout_workers = int(np.clip(action["num_rollout_workers"], config.num_rollout_workers_min, config.num_rollout_workers_max))
        num_envs_per_worker = int(np.clip(action["num_envs_per_worker"], config.num_envs_per_worker_min, config.num_envs_per_worker_max))
        rollout_fragment_length = config.envs[self.env_name]['rollout_fragment_length']
        if self.algo_name in ["ppo", "ppo_serverless"]:
            train_batch_size = num_rollout_workers * num_envs_per_worker * rollout_fragment_length
        else:
        ...
        # Update config (unfreeze to allow modification)
        self.trainer.config._is_frozen = False
        self.trainer.config.train_batch_size = train_batch_size
        self.trainer.config.num_rollout_workers = num_rollout_workers
        self.trainer.config.num_envs_per_worker = num_envs_per_worker
        self.trainer.config.sgd_minibatch_size = train_batch_size
        if self.algo_name in ["ppo", "ppo_serverless"]:
            self.trainer.config.serverless_batch_min = num_rollout_workers
            self.trainer.config.estimate_batch_size = num_rollout_workers * num_envs_per_worker * rollout_fragment_length
        else:
            self.trainer.config.serverless_batch_min = config.num_rollout_workers_min
        ...
        # Train one round
        print(f"[DEBUG] Starting trainer.train() round {round_id}...", flush=True)
        print(f"[DEBUG] num_rollout_workers={num_rollout_workers}, train_batch_size={train_batch_size}", flush=True)
        train_results = self.trainer.train()
        print(f"[DEBUG] trainer.train() completed for round {round_id}", flush=True)
        # print(train_results)
    ...
    def get_policy_state(self):
        return self.get_policy().get_state()
    
    def set_policy_state(self, policy_state):
        self.get_policy().set_state(policy_state)
    ...
    def invoke_local_actors(self, payload):
        from local_actor import local_actor_run
        local_actor_run(
            ...
            num_envs_per_worker=payload["num_envs_per_worker"],
            rollout_fragment_length=payload["rollout_fragment_length"],
            simulate_cold_start=config.local_simulate_cold_start,
        )
    ...
    def scale_serverless_actors(
        ...
        # Choose invocation target based on mode
        if self.is_local_simulation:
            from local_actor import local_actor_run
            target = local_actor_run
            actor_args = dict(
                redis_host=payload["redis_host"],
                redis_port=payload["redis_port"],
                redis_password=payload["redis_password"],
        ...
        print(f"[DEBUG] Launching {num_rollout_workers} local actors...", flush=True)
        jobs = []
        for i in range(num_rollout_workers):
            ...
            jobs.append(p)
            p.start()
            print(f"[DEBUG] Actor {i} started (pid={p.pid})", flush=True)

        for i, p in enumerate(jobs):
            p.join(timeout=120)
            if p.is_alive():
                print(f"[DEBUG] Actor {i} (pid={p.pid}) timed out, terminating", flush=True)
                p.terminate()
            elif p.exitcode != 0:
                print(f"[DEBUG] Actor {i} (pid={p.pid}) exited with code {p.exitcode}", flush=True)
            else:
                print(f"[DEBUG] Actor {i} (pid={p.pid}) completed OK", flush=True)
